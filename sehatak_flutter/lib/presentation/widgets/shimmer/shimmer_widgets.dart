  ...
  Widget build(BuildContext context) {
    return Column(children: [
      Expanded(child: ListView.builder(padding: const EdgeInsets.all(12), itemCount: 6, itemBuilder: (_, i) {
        final isMe = i % 2 == 0;
        return Align(alignment: isMe ? Alignment.centerRight : Alignment.centerLeft, child: Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: isMe ? AppColors.primary.withOpacity(0.1) : Colors.grey.shade100, borderRadius: BorderRadius.only(topLeft: const Radius.circular(18), topRight: const Radius.circular(18), bottomLeft: isMe ? const Radius.circular(18) : Radius.zero, bottomRight: isMe ? Radius.zero : const Radius.circular(18))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [ShimmerContainer(width: isMe ? 120 : 180, height: 12), SizedBox(height: 4), ShimmerContainer(width: isMe ? 80 : 60, height: 8)])));
      })),
...
class MoreShimmer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return SingleChildScrollView(padding: const EdgeInsets.all(14), child: Column(children: [
      SizedBox(height: 20),
      Center(child: Column(children: [const ShimmerContainer(width: 80, height: 80, borderRadius: 40), SizedBox(height: 12), const ShimmerContainer(width: 150, height: 18), SizedBox(height: 4), const ShimmerContainer(width: 100, height: 12)])),
      SizedBox(height: 20),
