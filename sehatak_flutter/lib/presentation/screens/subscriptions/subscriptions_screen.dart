...
class _SubscriptionsScreenState extends State<SubscriptionsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _planCard(String title, String emoji, String price, String period, Color color, List<String> features, bool selected, {bool isPopular = false, String? discount}) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Row(children: [Text(emoji, style: const TextStyle(fontSize: 30)), SizedBox(width: 10), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: color)), if (discount != null) Text(discount, style: const TextStyle(fontSize: 10, color: AppColors.success))])), Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('$price ر.ي', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: color)), Text('/$period', style: const TextStyle(fontSize: 10, color: AppColors.grey))])]),
  ...
  Widget _consultCard(String emoji, String title, String desc, String price, Color color) {
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 6)]), child: Row(children: [
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.end, children: [Text('$price ر.ي', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: color)), SizedBox(height: 4), ElevatedButton(onPressed: () => _openPayment(title, price, emoji), style: ElevatedButton.styleFrom(backgroundColor: color, foregroundColor: Colors.white, minimumSize: const Size(70, 28), padding: const EdgeInsets.symmetric(horizontal: 10), textStyle: const TextStyle(fontSize: 11)).copyWith(elevation: MaterialStateProperty.all(0)), child: const Text('احجز'))]),
    ]));
  }
  ...
  Widget _offerCard(String emoji, String title, String desc, String oldPrice, String newPrice, Color color, String badge) {
    return Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14), decoration: BoxDecoration(gradient: LinearGradient(colors: [color.withOpacity(0.05), color.withOpacity(0.02)]), borderRadius: BorderRadius.circular(14), border: Border.all(color: color.withOpacity(0.15))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Row(children: [Text(emoji, style: const TextStyle(fontSize: 28)), SizedBox(width: 10), Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))), Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3), decoration: BoxDecoration(color: color.withOpacity(0.1), borderRadius: BorderRadius.circular(20)), child: Text(badge, style: TextStyle(fontSize: 9, color: color)))]),
      SizedBox(height: 4), Text(desc, style: const TextStyle(fontSize: 11, color: AppColors.grey)),
      SizedBox(height: 10),
      Row(children: [Text(oldPrice, style: const TextStyle(fontSize: 13, color: AppColors.grey, decoration: TextDecoration.lineThrough)), SizedBox(width: 8), Text(newPrice, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: color)), const Spacer(), ElevatedButton(onPressed: () => _openPayment(title, newPrice, emoji), style: ElevatedButton.styleFrom(backgroundColor: color, foregroundColor: Colors.white, minimumSize: const Size(90, 30), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), child: const Text('استفد الآن'))]),
    ]));
  }

  Widget _buildMyAccountTab() {
    return SingleChildScrollView(padding: const EdgeInsets.all(14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('اشتراكي الحالي', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), SizedBox(height: 12),
      Container(padding: const EdgeInsets.all(20), decoration: BoxDecoration(gradient: const LinearGradient(colors: [AppColors.amber, Color(0xFFFF8F00)]), borderRadius: BorderRadius.circular(16)), child: Column(children: [const Icon(Icons.workspace_premium, color: Colors.white, size: 50), SizedBox(height: 8), const Text('الباقة الذهبية', style: TextStyle(color: Colors.white, fontSize: 22, fontWeight: FontWeight.bold)), const Text('سارية حتى 6 يونيو 2026', style: TextStyle(color: Colors.white70, fontSize: 13)), SizedBox(height: 12), Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(10)), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.check_circle, color: Colors.white, size: 18), SizedBox(width: 6), Text('الدفع التالي: 6 يونيو - 4,900 ر.ي', style: TextStyle(color: Colors.white, fontSize: 12))]))])),
      SizedBox(height: 16),
      const Text('طرق الدفع', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), SizedBox(height: 8),
      _payMethod('💳', 'فلوسك', '**** 4582'),
      _payMethod('💰', 'كاش', '**** 7891'),
      _payMethod('📱', 'جوالي', '**** 3456'),
      SizedBox(height: 20),
      const Text('إدارة الاشتراك', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)), SizedBox(height: 8),
  ...
  Widget _payMethod(String emoji, String name, String number) {
    return Container(margin: const EdgeInsets.only(bottom: 8), padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(10), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.02), blurRadius: 4)]), child: Row(children: [Text(emoji, style: const TextStyle(fontSize: 24)), SizedBox(width: 10), Expanded(child: Text(name, style: const TextStyle(fontWeight: FontWeight.bold))), Text(number, style: const TextStyle(color: AppColors.grey, fontSize: 12))]));
  }
